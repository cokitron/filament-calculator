...
import { PrintJob, LaborStage, JobFilament, Filament, Printer } from "../types";
import {
  computeCosts,
  formatMXN,
  formatMXNPrecise,
...
export default function Calculator({
  ...
  onAddToQueue: (job: PrintJob) => void;
  filaments: Filament[];
  printers: Printer[];
  settings: Settings;
}) {
  ...
  const handlePrinterSelect = (printerId: string) => {
    ...
    if (p) {
      set("printerId", p.id);
      set("powerWatts", p.powerWatts);
      set("machineRate", machineRate(p));
    } else {
      ...
      // using the printer-cost feature at all (Requirement 6.7).
    }
  };
  ...
  const removeFilament = (i: number) =>
    set(
      "jobFilaments",
      form.jobFilaments.filter((_, idx) => idx !== i),
    );
  const updateFilament = (
    ...
    newF[i] = {
      ...
      [k]: v,
      filamentId:
        k === "material" || k === "color" || k === "pricePerKg"
          ? ""
          : newF[i].filamentId,
    };
  ...
  const addStage = () =>
    set("laborStages", [
      ...form.laborStages,
      { name: "", hours: 1, rate: 100, scope: "per_unit" as const },
    ]);
  const removeStage = (i: number) =>
    set(
      "laborStages",
      form.laborStages.filter((_, idx) => idx !== i),
    );
  const updateStage = (i: number, k: keyof LaborStage, v: string | number) =>
    set(
      "laborStages",
      form.laborStages.map((s, idx) => (idx === i ? { ...s, [k]: v } : s)),
    );
  ...
  const costs = computeCosts(mockJob, settings);
      ...
      <div className="calc-layout">
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="ident-grid">
              <Field label="JOB NAME" error={error}>
                <Input
                  ...
                  placeholder="e.g. Mechanical Enclosure"
                />
              </Field>
              <Field label="CLIENT">
                <Input
              ...
              <Field label="QTY">
                <Input
                  ...
                  value={form.quantity}
                  onChange={(e) =>
                    set("quantity", Math.max(1, +e.target.value))
                  }
                />
              </Field>
              <Field label="PRINTER">
          ...
          <div style={blockStyle}>
                    ...
                    >
                      ...
                      {filaments.map((inv) => (
                        <option key={inv.id} value={inv.id}>
                          {inv.brand} {inv.type} - {inv.color} (
                          {formatMXN(inv.pricePerKg)}/kg)
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="mat-grid">
                    <Field label="MATERIAL">
                      <Input
                        ...
                        disabled={!!f.filamentId}
                        style={{
                          opacity: f.filamentId ? 0.5 : 1,
                          padding: "8px",
                        }}
                      />
                    </Field>
                    <Field label="COLOR">
                      <Input
                        value={f.color}
                        onChange={(e) =>
                          updateFilament(i, "color", e.target.value)
                        }
                        placeholder="Black"
                        disabled={!!f.filamentId}
                        style={{
                    ...
                    <Field label="MXN/KG">
                      <Input
                        ...
                        value={f.pricePerKg}
                        onChange={(e) =>
                          updateFilament(i, "pricePerKg", +e.target.value)
                        }
                        disabled={!!f.filamentId}
                        style={{
                          opacity: f.filamentId ? 0.5 : 1,
            ...
            >
              ...
              >
                {formatMXNPrecise(costs.filament)}
              </div>
            </div>
          </div>
            ...
            <div className="field-grid-2 mb-6">
              ...
              <Field label="MISC FINISHING (MXN / UNIT)">
                <Input
                  ...
                  onChange={(e) => set("finishingCost", +e.target.value)}
                />
              </Field>
            </div>
              ...
              {(form.laborStages || []).map((stage, i) => (
                ...
                >
                  ...
                  <input
                    ...
                    step={0.25}
                    value={stage.hours}
                    onChange={(e) => updateStage(i, "hours", +e.target.value)}
                    style={inlineInput}
                    aria-label="Horas"
                  />
                  <span
                    className="labor-unit"
                    style={{
                      fontFamily: "var(--font-mono)",
                      fontSize: 12,
                  ...
                  <input
                    ...
                    step={1}
                    value={stage.rate}
                    onChange={(e) => updateStage(i, "rate", +e.target.value)}
                    style={inlineInput}
        ...
        >
          ...
          <div className="flex flex-col gap-3 mb-6">
            ...
            <CostRow label="MACHINE" value={costs.machine} />
            ...
            >
              {form.quantity > 1 && (
                ...
                >
                  ...
                  >
                    {formatMXN(costs.unitPrice * form.quantity)}
                  </span>
                </div>
              )}
              {costs.laborPerJob > 0 && (
                ...
                >
                  ...
                  >
                    +
                    {formatMXN(
                      costs.laborPerJob * (1 + (form.marginPercent || 0) / 100),
                    )}
                  </span>
                </div>
              )}
              ...
              >
                ...
                <span
                  style={{
                    ...
                    fontSize: 11,
                    color: "#000",
                    opacity: form.ivaEnabled ? 1 : 0.45,
                  }}
                >
                  +{formatMXN(costs.iva)}
                </span>
...
function CostRow({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex justify-between items-center">
      <span
        style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "#666" }}
      >
        {label}
      </span>
      <span
