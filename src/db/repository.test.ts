...
describe("settings", () => {
  ...
  it("round-trips minimumOrder", () => {
    updateSettings(db, { minimumOrder: 150 });
    expect(getSettings(db).minimumOrder).toBe(150);
  });
});
...
function aPrinter(overrides: Partial<Omit<Printer, "id" | "status">> = {}) {
  return {
    name: "Ender 3",
    model: "V2",
    powerWatts: 220,
...
describe("printers", () => {
  it("creates and lists active printers, ordered by name", () => {
    createPrinter(db, aPrinter({ name: "Zeta" }));
    createPrinter(db, aPrinter({ name: "Alpha" }));

    const all = listPrinters(db);
  ...
  it("rejects createPrinter when amortizationHours is zero or negative, writing no row", () => {
    expect(() => createPrinter(db, aPrinter({ amortizationHours: 0 }))).toThrow(
      /amortizationHours/,
    );
    expect(() =>
      createPrinter(db, aPrinter({ amortizationHours: -10 })),
    ).toThrow(/amortizationHours/);
    expect(listPrinters(db, true)).toHaveLength(0);
  });

  it("rejects createPrinter when powerWatts, purchaseCost, or maintenanceCostPerHour is negative, writing no row", () => {
    expect(() => createPrinter(db, aPrinter({ powerWatts: -1 }))).toThrow(
      /powerWatts/,
    );
    expect(() => createPrinter(db, aPrinter({ purchaseCost: -1 }))).toThrow(
      /purchaseCost/,
    );
    expect(() =>
      createPrinter(db, aPrinter({ maintenanceCostPerHour: -1 })),
    ).toThrow(/maintenanceCostPerHour/);
    expect(listPrinters(db, true)).toHaveLength(0);
  });

  it("rejects updatePrinter with the same invalid fields, without writing the row", () => {
    const id = createPrinter(db, aPrinter());
    const before = listPrinters(db, true)[0];

    expect(() => updatePrinter(db, id, { amortizationHours: 0 })).toThrow(
      /amortizationHours/,
    );
    expect(() => updatePrinter(db, id, { powerWatts: -5 })).toThrow(
      /powerWatts/,
    );
    expect(() => updatePrinter(db, id, { purchaseCost: -5 })).toThrow(
      /purchaseCost/,
    );
    expect(() => updatePrinter(db, id, { maintenanceCostPerHour: -5 })).toThrow(
      /maintenanceCostPerHour/,
    );

    // Property 5: Validation atomicity -- a rejected call leaves the row
    // exactly as it was before the call, for every field, not just the one
    // that would have violated a constraint.
    expect(listPrinters(db, true)[0]).toEqual(before);
  });

  it("updatePrinter only validates fields present in the patch", () => {
    const id = createPrinter(db, aPrinter());
    updatePrinter(db, id, { name: "Ender 3 Pro" });
    expect(listPrinters(db, true)[0]).toMatchObject({ name: "Ender 3 Pro" });
  });

  it("updatePrinter applies a valid patch", () => {
    const id = createPrinter(db, aPrinter());
    updatePrinter(db, id, { purchaseCost: 6200, maintenanceCostPerHour: 1.2 });
    const p = listPrinters(db, true)[0];
    expect(p.purchaseCost).toBe(6200);
    expect(p.maintenanceCostPerHour).toBe(1.2);
    // Untouched fields are left alone.
    expect(p.amortizationHours).toBe(3000);
  });

  it("excludes a retired printer from listPrinters(db, false) but keeps it visible with includeInactive", () => {
    const id = createPrinter(db, aPrinter({ name: "Retired One" }));
    setPrinterStatus(db, id, "retired");

    expect(listPrinters(db, false)).toHaveLength(0);
    const all = listPrinters(db, true);
    expect(all).toHaveLength(1);
    expect(all[0]).toMatchObject({ id, status: "retired" });
  });

  it("setPrinterStatus can move a printer back to active", () => {
    const id = createPrinter(db, aPrinter());
    setPrinterStatus(db, id, "maintenance");
    expect(listPrinters(db, false)).toHaveLength(0);
    setPrinterStatus(db, id, "active");
    expect(listPrinters(db, false)).toHaveLength(1);
  });

  it("Property 4: a job's snapshotted machineRate/powerWatts survive later edits to the printer", () => {
    const printerId = createPrinter(db, aPrinter({ powerWatts: 220 }));
    const job = aJob({
      printerId,
      powerWatts: 220,
      machineRate: 5000 / 3000 + 0.5, // matches aPrinter()'s purchaseCost/amortizationHours/maintenanceCostPerHour
    });
    saveJob(db, job);
    const before = getJob(db, job.id)!;

    updatePrinter(db, printerId, {
      purchaseCost: 999999,
      amortizationHours: 1,
      maintenanceCostPerHour: 999,
    });

    const after = getJob(db, job.id)!;
    expect(after.machineRate).toBe(before.machineRate);
    expect(after.powerWatts).toBe(before.powerWatts);
  });
});
...
describe("saveJob / listJobs", () => {
  ...
  it("defaults a scopeless stage to per_unit on the way in", () => {
    saveJob(
      db,
      aJob({ laborStages: [{ name: "Legacy", hours: 1, rate: 100 }] }),
    );
  ...
  it("rolls back the whole job if one child row is invalid", () => {
    const bad = aJob({
      laborStages: [{ name: "X", hours: -5, rate: 100, scope: "per_unit" }],
    });
  ...
  it("round-trips printerId/machineRate through getJob unchanged", () => {
    const printerId = createPrinter(db, aPrinter({ name: "Snapshot Printer" }));
    const job = aJob({ printerId, machineRate: 2.9 });
    saveJob(db, job);

    const loaded = getJob(db, job.id)!;
    expect(loaded.printerId).toBe(printerId);
    expect(loaded.machineRate).toBe(2.9);
  });
});
...
describe("active queue vs history", () => {
  ...
  it("stamps when the run started and when it closed", () => {
    ...
    saveJob(db, job);
    updateJobStatus(db, job.id, "printing");
    const started = db.get<{ started_at: string }>(
  ...
  it("does not reset the start stamp when a job is restarted", () => {
    ...
    saveJob(db, job);
    updateJobStatus(db, job.id, "printing");
    const first = db.get<{ started_at: string }>(
      "select started_at from jobs where id = ?",
      [job.id],
    )!.started_at;
  ...
  it("orders history by completion, newest first", () => {
    ...
    db.run(
      "update jobs set completed_at = '2026-03-01T00:00:00.000Z' where id = ?",
      [b.id],
    );
    updateJobStatus(db, a.id, "done");
    db.run(
      "update jobs set completed_at = '2026-04-01T00:00:00.000Z' where id = ?",
      [a.id],
    );

    expect(listJobHistory(db).map((e) => e.job.name)).toEqual([
      "Primero",
      "Segundo",
    ]);
  });
...
describe("duplicateJob", () => {
  it("re-quotes a past job as a new queued job", () => {
    ...
    expect(
      copy.jobFilaments.map((f) => [f.material, f.weight, f.pricePerKg]),
    ).toEqual(
