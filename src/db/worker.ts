...
function makeExecutor(db: any): SqlExecutor {
  ...
  return {
    ...
    transaction<T>(fn: () => T): T {
      if (depth > 0) throw new Error('SqlExecutor.transaction cannot be nested')
      depth++
      db.exec('begin')
      try {
        const result = fn()
...
async function open(): Promise<{ persistent: boolean; schema: { from: number; to: number } }> {
  if (executor) {
...
type Request =
  ...
  | { id: number; op: 'updateJobStatus'; jobId: string; status: PrintJob['status'] }
  | { id: number; op: 'deleteJob'; jobId: string; force?: boolean }
  | { id: number; op: 'duplicateJob'; jobId: string; name?: string }
  | { id: number; op: 'issueQuote'; jobId: string; status?: 'sent' | 'accepted' | 'rejected' }
