...
function makeQuote(folio = 1): string {
  ...
  db.run('insert into quotes (id, folio, created_at, updated_at) values (?, ?, ?, ?)', [
...
function makeJob(quoteId: string, quantity = 1): string {
  ...
  db.run(
    ...
    [id, quoteId, 'Job', quantity, '2026-01-01T00:00:00.000Z', '2026-01-01T00:00:00.000Z'],
  )
  return id
}
...
describe('schema initialisation', () => {
  ...
  it('creates every expected table', () => {
    ...
    expect(names).toEqual(expect.arrayContaining([
      'customers', 'filament_spools', 'filaments', 'job_actuals', 'job_labor_stages',
      'job_materials', 'jobs', 'printers', 'quotes', 'schema_version', 'settings',
    ]))
  })
})
...
describe('foreign keys', () => {
  ...
  it('nulls a job material filament reference instead of deleting history', () => {
    ...
    const row = db.get<{ filament_id: string | null; material_label: string; price_per_kg: number }>(
...
describe('settings', () => {
  it('is a singleton — a second row is rejected', () => {
    expect(() =>
      db.run('insert into settings (id, created_at, updated_at) values (2, ?, ?)', ['x', 'x']),
    ).toThrow()
  })

  it('defaults to MXN with IVA off at 16%', () => {
    const s = db.get<{ currency: string; default_iva_enabled: number; iva_rate: number; electricity_rate: number }>(
  ...
  it('ensureSettings does not overwrite existing values', () => {
    ...
    ensureSettings(db)
    const s = db.get<{ business_name: string }>('select business_name from settings')
...
describe('constraints', () => {
  ...
  it('accepts both valid labor scopes', () => {
    ...
    for (const scope of ['per_job', 'per_unit']) {
      db.run('insert into job_labor_stages (id, job_id, scope, created_at) values (?, ?, ?, ?)', [
  ...
  it('requires frozen totals once a quote leaves draft', () => {
    const id = makeQuote()
    expect(() => db.run('update quotes set status = ? where id = ?', ['sent', id])).toThrow()

    db.run(
      'update quotes set status = ?, total = ?, total_pre_tax = ?, total_iva = ?, frozen_at = ? where id = ?',
      ['sent', 1160, 1000, 160, '2026-01-02T00:00:00.000Z', id],
    )
    const q = db.get<{ status: string; total: number }>('select status, total from quotes where id = ?', [id])
  ...
  it('forbids a draft from carrying frozen totals', () => {
    const id = makeQuote()
    expect(() =>
      db.run('update quotes set frozen_at = ?, total = ? where id = ?', ['x', 100, id]),
    ).toThrow()
  })
...
describe('money storage', () => {
  it('round-trips peso values with no precision loss', () => {
    ...
    for (const v of values) {
      db.run('insert into job_materials (id, job_id, grams, price_per_kg, created_at) values (?, ?, ?, ?, ?)', [
        crypto.randomUUID(), jobId, 1, v, 'x',
      ])
    }
    const stored = db.all<{ price_per_kg: number }>('select price_per_kg from job_materials order by rowid')
...
describe('transactions', () => {
  ...
  it('commits when the body succeeds', () => {
    const quoteId = makeQuote()
    db.transaction(() => { makeJob(quoteId) })
...
describe('v_job_overview', () => {
  it('joins job, quote, customer and printer', () => {
    ...
    db.run('insert into printers (id, name, created_at, updated_at) values (?, ?, ?, ?)', [
  ...
  it('still returns the job when there is no customer', () => {
    makeJob(makeQuote())
    const row = db.get<{ customer_name: string }>('select * from v_job_overview')
