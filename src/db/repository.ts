...
export interface Settings {
  ...
  /** MXN floor applied to a job's total price. 0 means no minimum is enforced. */
  minimumOrder: number;
}
...
export function getSettings(db: SqlExecutor): Settings {
  ...
  return {
    ...
    minimumOrder: (row.minimum_order as number) ?? 0,
  };
}
...
export function updateFilamentPrice(
  db: SqlExecutor,
  id: string,
  pricePerKg: number,
): void {
...
export function deactivateFilament(db: SqlExecutor, id: string): void {
  db.run("update filaments set is_active = 0, updated_at = ? where id = ?", [
    nowIso(),
    id,
  ]);
}
...
export function listPrinters(
  ...
  const rows = db.all<Record<string, unknown>>(
    `select id, name, model, power_watts, purchase_cost, amortization_hours,
            maintenance_cost_per_hour, status
     from printers
  ...
  return rows.map((r) => ({
    ...
    amortizationHours: r.amortization_hours as number,
    maintenanceCostPerHour: r.maintenance_cost_per_hour as number,
    status: r.status as PrinterStatus,
  }));
}

/**
 * Validate the fields a create/update request supplies. Only checks keys
 * present in `fields`, so updatePrinter can validate a partial patch the same
 * way createPrinter validates a full record.
 */
function validatePrinterFields(
  fields: Partial<
    Pick<
      Printer,
      | "amortizationHours"
  ...
  if (fields.amortizationHours !== undefined && fields.amortizationHours <= 0) {
    throw new Error("amortizationHours must be greater than 0");
  }
  if (fields.powerWatts !== undefined && fields.powerWatts < 0) {
    throw new Error("powerWatts must not be negative");
  }
  if (fields.purchaseCost !== undefined && fields.purchaseCost < 0) {
    throw new Error("purchaseCost must not be negative");
  }
  if (
    fields.maintenanceCostPerHour !== undefined &&
    fields.maintenanceCostPerHour < 0
  ) {
    throw new Error("maintenanceCostPerHour must not be negative");
  }
}

export function createPrinter(
  db: SqlExecutor,
  p: Omit<Printer, "id" | "status"> & { id?: string },
): string {
  validatePrinterFields(p);
...
export function updatePrinter(
  ...
  validatePrinterFields(patch);

  const columns: Record<keyof Omit<Printer, "id" | "status">, string> = {
    name: "name",
    model: "model",
    powerWatts: "power_watts",
    purchaseCost: "purchase_cost",
    amortizationHours: "amortization_hours",
  ...
  params.push(nowIso());
  params.push(id);
  db.run(`update printers set ${sets.join(", ")} where id = ?`, params);
}
...
export function setPrinterStatus(
  ...
  db.run("update printers set status = ?, updated_at = ? where id = ?", [
    status,
    nowIso(),
    id,
  ]);
...
function nextFolio(db: SqlExecutor): number {
  const row = db.get<{ next: number }>(
    "select coalesce(max(folio), 0) + 1 as next from quotes",
  );
  return row?.next ?? 1;
}
...
export function saveJob(db: SqlExecutor, job: PrintJob): string {
  return db.transaction(() => {
    ...
    const existing = db.get<{ quote_id: string }>(
      "select quote_id from jobs where id = ?",
      [job.id],
    );
    ...
    if (existing) {
      ...
      db.run(
        `update jobs set name = ?, quantity = ?, print_time_hours = ?, power_watts = ?,
                         packaging_cost = ?, finishing_cost = ?, status = ?, notes = ?,
                         printer_id = ?, machine_rate = ?, updated_at = ?
         where id = ?`,
        [
          job.name,
          job.quantity,
      ...
      db.run(
        `insert into jobs (id, quote_id, name, quantity, print_time_hours, power_watts,
                           packaging_cost, finishing_cost, status, notes,
                           printer_id, machine_rate, created_at, updated_at)
         values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          job.id,
          quoteId,
...
function jobWhere(filter: JobFilter): { clause: string; params: SqlParam[] } {
  ...
  if (filter.statuses && filter.statuses.length > 0) {
    conditions.push(
      `j.status in (${filter.statuses.map(() => "?").join(", ")})`,
    );
...
export function updateJobStatus(
  ...
  db.transaction(() => {
    ...
    const startedAt =
      status === "printing" ? (row.started_at ?? ts) : row.started_at;
...
export function deleteJob(
  ...
  db.transaction(() => {
    ...
    const siblings = db.get<{ n: number }>(
      "select count(*) as n from jobs where quote_id = ?",
      [row.quote_id],
    );
    if ((siblings?.n ?? 0) === 0) {
...
export function duplicateJob(
  ...
  const copy: PrintJob = {
    ...
    jobFilaments: (source.jobFilaments ?? []).map((m) => ({
      ...m,
      id: crypto.randomUUID(),
    })),
...
function freezeQuote(
  ...
  const costs = computeCosts(job, getSettings(db));
